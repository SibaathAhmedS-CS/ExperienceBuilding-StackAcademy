...
import { normalizeArray } from '@/types/contentstack';
  ...
  useEffect(() => {
    async function fetchCertificateData() {
      try {
        ...
        // ✅ Security Check 1: Fetch enrollment by ID and verify ownership
        ...
        // ✅ Security Check 2: Verify enrollment exists, belongs to current user, and is completed
        if (!enrollment || enrollment.user_id !== user.id || enrollment.status !== 'completed') {
          // Redirect to courses page if not authorized or not completed
          router.push('/courses');
          return;
        }

        // Fetch course data to verify lesson completion
        ...
        // ✅ Security Check 3: Verify all lessons are completed (double-check)
        ...
        if (!lessonProgress || lessonProgress.length < totalLessons) {
          // Redirect to course page if not all lessons completed
          router.push(`/course/${course.slug}`);
          return;
        }

        // All checks passed - fetch user profile and set certificate data
        ...
        setCertificateData({
          ...
          certificateId: `CERT-${enrollment.course_id.substring(0, 6)}-${user.id.substring(0, 4)}`.toUpperCase()
        });
      } catch (err) {
