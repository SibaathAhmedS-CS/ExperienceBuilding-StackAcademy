...
function setupLyticsListeners() {
  ...
  jstag.on('profile', function(profile) {
    ...
    // Check for segments in cookie
    var lySegsCookie = document.cookie.split('; ').find(function(row) {
      return row.startsWith('ly_segs=');
    });
    if (lySegsCookie) {
      var segments = lySegsCookie.split('=')[1];
      var segmentArray = segments ? segments.split(',').filter(Boolean) : [];
      if (segmentArray.length > 0) {
        console.log('[Lytics] 🍪 Segments from cookie after profile update:', JSON.stringify(segmentArray, null, 2));
        if (typeof window !== 'undefined' && window.dispatchEvent) {
          window.dispatchEvent(new CustomEvent('lytics:segmentsUpdated', {
